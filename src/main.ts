...
function initDeviceObjects(deviceId, channels, data) {
  ...
  if (data && data.electricity) {
    ...
    for (let key in data.electricity) {
      if (!data.electricity.hasOwnProperty(key)) {
        continue;
      }
      if (key === 'channel') {
        continue;
      ...
      if (roleValues[key] && roleValues[key].unit) {
  ...
  if (data && data.light) {
    for (let key in data.light) {
      if (!data.light.hasOwnProperty(key)) {
        continue;
      }
      if (key === 'channel' || key === 'capacity') {
        continue;
      }
      const common = {};
  ...
  if (data && data.hub) {
    ...
    data.hub.subdevice.forEach((sub) => {
      ...
      if (sub.mts100 || sub.mts100v3) {
        ...
        knownDevices[deviceId].deviceAbilities &&
        knownDevices[deviceId].deviceAbilities.ability['Appliance.Hub.Sensor.TempHum'] &&
        sub.ms100
      ) {
        common = {};
        common.type = 'number';
...
function initDevice(deviceId, deviceDef, device, callback) {
  ...
  device.getSystemAbilities((err, deviceAbilities) => {
    ...
    device.getSystemAllData((err, deviceAllData) => {
      ...
      if (
        !deviceAbilities.ability['Appliance.Control.ToggleX'] &&
        !deviceAbilities.ability['Appliance.Control.Toggle'] &&
        !deviceAbilities.ability['Appliance.Control.Electricity'] &&
        !deviceAbilities.ability['Appliance.GarageDoor.State'] &&
        !deviceAbilities.ability['Appliance.Control.Light'] &&
        !deviceAbilities.ability['Appliance.Digest.Hub'] &&
        !deviceAbilities.ability['Appliance.Control.Spray']
      ) {
      ...
      if (
        deviceAllData &&
        deviceAllData.all &&
        deviceAllData.all.system &&
        deviceAllData.all.system.firmware &&
...
function pollElectricity(deviceId, delay) {
  ...
  if (!delay) {
    delay = adapter.config.electricityPollingInterval || 20;
  }
  ...
  if (payload && payload.tempHum) {
    ...
    payload.tempHum.forEach((val) => {
      if (val.latestTemperature !== undefined) {
        adapter.setState(
          deviceId + '.' + val.id + '.latestTemperature',
          {
      ...
      if (val.latestHumidity !== undefined) {
        adapter.setState(
          deviceId + '.' + val.id + '.latestHumidity',
          {
...
function setValuesLight(deviceId, payload) {
  ...
  if (payload && payload.light) {
    for (let key in payload.light) {
      if (!payload.light.hasOwnProperty(key)) {
        continue;
      }
      if (key === 'channel' || key === 'capacity') {
        continue;
      ...
      if (key === 'rgb') {
        payload.light[key] = convertNumberToHex(payload.light[key]);
      }
...
function setValuesElectricity(deviceId, payload) {
  ...
  if (payload && payload.electricity) {
    ...
    for (let key in payload.electricity) {
      if (!payload.electricity.hasOwnProperty(key)) {
        continue;
      }
      if (key === 'channel') {
        continue;
      ...
      adapter.setState(
        deviceId + '.' + channel + '-' + key,
        Math.floor(payload.electricity[key] * Math.pow(10, roleValues[key] ? roleValues[key].scale || 0 : 0) * 100) / 100,
        true,
      );
    }
  }
